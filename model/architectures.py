...
class FR_Encoder(nn.Module):
    def __init__(self, output_size, input_size=(1, 64, 64)):
        ...
        self.conv = nn.Sequential(
            ...
            nn.Conv2d(self.channel_mult*1, self.channel_mult*2, 4, 2, 1),  # self.channel_mult*2 x 32 x 32
            nn.BatchNorm2d(self.channel_mult*2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(self.channel_mult*2, self.channel_mult*4, 4, 2, 1),  # self.channel_mult*4 x 16 x 16
            nn.BatchNorm2d(self.channel_mult*4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(self.channel_mult*4, self.channel_mult*8, 4, 2, 1),  # self.channel_mult*8 x 8 x 8
            nn.BatchNorm2d(self.channel_mult*8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(self.channel_mult*8, self.channel_mult*16, 3, 2, 1), # self.channel_mult*16 x 4 x 4
            nn.BatchNorm2d(self.channel_mult*16),
            nn.LeakyReLU(0.2, inplace=True)
        )

    def forward(self, x):
        return  self.conv(x.view(-1, *self.input_size))


class FR_Decoder(nn.Module):
    def __init__(self, embedding_size, input_size=(1, 64, 64)):
        super(FR_Decoder, self).__init__()
